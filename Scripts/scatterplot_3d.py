...
def analyze_masks_and_tiffs_3d(masks_dir, tiffs_dir, band1_idx, band2_idx, band3_idx, output_dir, 
                              class_labels, band_names, class_colors, points_per_image=10000,
                              combine_images=False, alpha=0.4):
    """
    Analyze mask and TIF file pairs to create 3D Band visualizations.
    ...
    """
    combined_data = {'class': [], 'band1': [], 'band2': [], 'band3': [], 'image': []}
    ...
    for mask_filename in os.listdir(masks_dir):
        ...
        with rasterio.open(tif_path) as tif_src:
            ...
            band3 = normalize_band(tif_src.read(band3_idx))
    ...
    if combine_images:
        ...
        _print_statistics(combined_data)

def _create_3d_scatter_plot(df, band1_idx, band2_idx, band3_idx, band_names, class_colors, alpha, output_dir, filename_prefix):
    """Helper function to create and save 3D scatter plots."""
    ...
    title = f'3D Distribution of {band_names.get(band1_idx, "Band1")}, '
    title += f'{band_names.get(band2_idx, "Band2")}, and '
    title += f'{band_names.get(band3_idx, "Band3")} Values by Class'
    ax.set_title(title)
    
    ax.legend(bbox_to_anchor=(1.15, 1), loc='upper right')
    plt.tight_layout()
    
    output_filename = (f"{filename_prefix}_{band_names.get(band1_idx, f'Band{band1_idx}')}_"
                      f"{band_names.get(band2_idx, f'Band{band2_idx}')}_"
                      f"{band_names.get(band3_idx, f'Band{band3_idx}')}_Band_Comparison_3d.png")
...
def _print_statistics(data, image_name=None, last_n=None):
    """Helper function to print statistics for the data."""
    df = pd.DataFrame(data)
    if last_n is not None:
        df = df.tail(last_n)
    
    image_str = f" for {image_name}" if image_name else ""
    print(f"\nBand Value Statistics by Class{image_str}:")
...
if __name__ == "__main__":
    ...
    analyze_masks_and_tiffs_3d(
        ...
        points_per_image=2000,    # Number of points to sample per class per image
